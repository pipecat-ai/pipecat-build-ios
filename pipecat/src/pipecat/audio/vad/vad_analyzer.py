...
class VADAnalyzer(ABC):
    ...
    def _update_vad_state(self, confidence: float, volume: float) -> None:
        """Advance the debounce counters by one confidence/volume observation."""
    ...
    def _finalize_vad_state(self) -> VADState:
        """Confirm pending transitions after an observation or an audio batch."""
        if (
            self._vad_state == VADState.STARTING
