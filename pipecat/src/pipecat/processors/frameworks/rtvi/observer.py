...
class RTVIObserverParams:
    ...
    Parameters:
        ...
        sentence_boundary_matcher: Optional sentence boundary function for legacy
            bot transcription messages. Returns the sentence-end offset, or zero
            when no complete sentence is available. Defaults to Pipecat's matcher.
...
class RTVIObserver(BaseObserver):
    ...
    def requires_rtvi_processor(self) -> bool:
        """Whether this observer sends messages through an RTVIProcessor.

        Observers that override send_rtvi_message to use their own native
        transport can return False and run without an RTVIProcessor.
        """
        return True
    ...
    async def _handle_llm_text_frame(self, frame: LLMTextFrame):
        ...
        matcher = self._params.sentence_boundary_matcher or match_endofsentence
        if self._bot_transcription and matcher(self._bot_transcription):
