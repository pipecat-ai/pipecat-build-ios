...
class LLMTextProcessor(FrameProcessor):
    ...
    async def process_frame(self, frame: Frame, direction: FrameDirection):
        ...
        if isinstance(frame, InterruptionFrame):
            ...
            await self._handle_llm_end(frame.skip_tts, metadata=frame.metadata)
            await self.push_frame(frame, direction)
        elif isinstance(frame, EndFrame):
            await self._handle_llm_end(metadata=frame.metadata)
            await self.push_frame(frame, direction)
        else:
            await self.push_frame(frame, direction)
    ...
    async def _handle_llm_text(self, in_frame: LLMTextFrame):
        async for aggregation in self._text_aggregator.aggregate(in_frame.text):
            ...
            out_frame.metadata.update(in_frame.metadata)
            await self.push_frame(out_frame)

    async def _handle_llm_end(self, skip_tts: bool | None = None, *, metadata: dict | None = None):
        ...
        if remaining:
            ...
            out_frame.metadata.update(metadata or {})
            await self.push_frame(out_frame)
